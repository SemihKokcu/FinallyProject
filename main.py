...
hand_landmark = hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.6, max_num_hands=1)
...
while True:
        ...
        for i in op.multi_hand_landmarks:
            ...
            if curr_tool == "Ciz":
                ...
                if index_raised(yi, y9):
                    ...
                    cv2.circle(background, (x, y), 70, (255, 255, 255), -1)
    ...
    op = cv2.bitwise_and(background, background, mask=mask)
    background[:, :, 1] = op[:, :, 1]
    background[:, :, 2] = op[:, :, 2]


    cv2.putText(background,"Beyaz Tahta", (270 + ml, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1)
